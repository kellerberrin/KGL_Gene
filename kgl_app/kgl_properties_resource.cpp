...
kgl::ResourceDefinitions kgl::ResourceProperties::getRuntimeResources() const {
  ...
  for (const auto &[tree_type, sub_tree]: property_tree_vector) {
    ...
    switch (Utility::hash64(tree_type)) {

      case Utility::hash64(GENOME_RESOURCE_ID_):
        resource_opt = genomeResourceXML(sub_tree);
        break;

      case Utility::hash64(ONTOLOGY_RESOURCE_ID_):
        resource_opt = ontologyResourceXML(sub_tree);
        break;

      case Utility::hash64(GENE_NOMENCLATURE_RESOURCE_ID_):
        resource_opt = geneIDResourceXML(sub_tree);
        break;

      case Utility::hash64(GENEALOGY_RESOURCE_ID_):
        resource_opt = genealogyIDResourceXML(sub_tree);
        break;

      case Utility::hash64(CITATION_RESOURCE_ID_):
        resource_opt = citationResourceXML(sub_tree);
        break;

      case Utility::hash64(ENTREZ_RESOURCE_ID_):
        resource_opt = entrezResourceXML(sub_tree);
        break;

      case Utility::hash64(PF7SAMPLE_RESOURCE_ID_):
        resource_opt = Pf7SampleResourceXML(sub_tree);
        break;

      case Utility::hash64(PF7FWS_RESOURCE_ID_):
        resource_opt = Pf7FwsResourceXML(sub_tree);
        break;

      case Utility::hash64(PF7DISTANCE_RESOURCE_ID_):
        resource_opt = Pf7DistanceResourceXML(sub_tree);
        break;

      case Utility::hash64(PUBMED_API_RESOURCE_ID_):
        resource_opt = PubmedLitAPIResourceXML(sub_tree);
        break;

      case Utility::hash64(GENOMEAUX_RESOURCE_ID_):
        resource_opt = auxIDResourceXML(sub_tree);
        break;

      case Utility::hash64(PF3K_COI_RESOURCE_ID_):
