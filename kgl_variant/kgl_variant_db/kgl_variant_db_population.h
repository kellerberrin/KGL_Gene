...
//////////////////////////////////////////////////////////////////////////////////////////////////////
//
// The population level contains genomes (organisms), which in turn contain contigs (chromosomes),
// which turn contain offsets, which in turn contain an array of variants.
...
/// The population level contains genomes (organisms), which in turn contain contigs (chromosomes),
/// which turn contain offsets, which in turn contain an array of variants.
/// Note that the population structure cannot be read while it is being updated (multi-threaded parsing).
...
public:
  ...
  ~PopulationDB() override { genome_map_.clear(); }  // Quicker than relying on smart pointer reference counting.
  ...
  [[nodiscard]] std::map<ContigId_t , size_t> contigCountMap() const;

  /// Returns the total contig variant count for a population.
  /// Returns std::nullopt if the contig is not present in the population.
  [[nodiscard]] std::optional<size_t> contigCount(const ContigId_t& contig) const;
  ...
  /// Note that if a genome id occurs twice (homozygous) in the genome vector the variant will be
