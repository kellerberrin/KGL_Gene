...
void kgl::dEploidExecEnv::executeApp() {
  ...
  kgl::dEploid(args.argc, args.argv);
...
bool kgl::dEploidExecEnv::parseCommandLine(int argc, char const ** argv) {
  ...
  setShortDescription(parser, "dEploid COI haplotype generation");
  setVersion(parser, VERSION);
  setDate(parser, "May 2018");

  const char* program_desc =
      R"(dEploid Complexity Of Infection (COI) haplotype generation)";
  ...
  const char* newLogFileShortFlag_ = "nnn";
