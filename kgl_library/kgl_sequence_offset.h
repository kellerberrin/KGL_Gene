...
// Convenience routine that returns a coding sequence from an unmutated (reference) sequence
  [[nodiscard]] static std::shared_ptr<DNA5SequenceCoding> refCodingSubSequence( std::shared_ptr<const CodingSequence> coding_seq_ptr,
...
// Returned sequences are in transcription (strand) order with array[0] being the first exon.
  [[nodiscard]] static std::vector<std::shared_ptr<DNA5SequenceCoding>> refExonArraySequence( std::shared_ptr<const CodingSequence> coding_seq_ptr,
                                                                                              const DNA5SequenceLinear& sequence,
                                                                                              ContigOffset_t sub_sequence_offset,
                                                                                              ContigSize_t sub_sequence_length,
                                                                                              ContigOffset_t contig_offset);
...
// Setting sub_sequence_offset and sub_sequence_length to zero copies the entire intron sequence defined by the SortedCDS.
  [[nodiscard]] static std::shared_ptr<DNA5SequenceCoding> refIntronSubSequence( std::shared_ptr<const CodingSequence> coding_seq_ptr,
...
// Returned sequences are in transcription (strand) order with array[0] being the first intron.
  [[nodiscard]] static std::vector<std::shared_ptr<DNA5SequenceCoding>> refIntronArraySequence( std::shared_ptr<const CodingSequence> coding_seq_ptr,
                                                                                                const DNA5SequenceLinear& sequence,
                                                                                                ContigOffset_t sub_sequence_offset,
                                                                                                ContigSize_t sub_sequence_length,
                                                                                                ContigOffset_t contig_offset);
  ...
  // Convenience routine that returns a coding sequence from an unmutated (reference) sequence
  [[nodiscard]] static std::shared_ptr<DNA5SequenceCoding> mutantCodingSubSequence( std::shared_ptr<const CodingSequence> coding_seq_ptr,
                                                                                    const DNA5SequenceLinear& sequence,
...
// The offset is adjusted for strand type; the offset arithmetic is reversed for -ve strand sequences.
  [[nodiscard]] static bool refOffsetWithinCodingSequence( std::shared_ptr<const CodingSequence> coding_seq_ptr,
                                                           ContigOffset_t contig_offset,
...
// The contig offset is adjusted for strand type; the offset arithmetic is reversed for -ve strand sequences.
  [[nodiscard]] static bool refCodingSequenceContigOffset( std::shared_ptr<const CodingSequence> coding_seq_ptr,
                                                           ContigOffset_t coding_sequence_offset,
                                                           ContigOffset_t &contig_offset,
                                                           ContigSize_t &coding_sequence_length);
...
// In general the number of intron offsets returned is the number of defined exons - 1
  [[nodiscard]] static bool intronOffsetAdapter( std::shared_ptr<const CodingSequence> coding_seq_ptr,
                                                 StrandSense& strand,
                                                 IntronOffsetMap& intron_offset_map);

  // Converts linear DNA to a coding DNA sequence.
  [[nodiscard]] static std::shared_ptr<DNA5SequenceCoding> codingSequence(std::shared_ptr<const DNA5SequenceLinear> base_sequence, StrandSense strand);
...
private:
  ...
  [[nodiscard]] static std::shared_ptr<DNA5SequenceCoding> codingSubSequence( const DNA5SequenceLinear& base_sequence,
...
// The offsets are initially contig offsets. However these can be modified to account for indel offset modification.
// The map is generated by an adapter function exonOffsetAdapter() that uses a coding sequence as input.
  [[nodiscard]] static bool exonOffsetAdapter( std::shared_ptr<const CodingSequence> coding_seq_ptr,
                                               StrandSense& strand,
                                               ExonOffsetMap& exon_offset_map);

  // Adjusted for indel mutations.
  [[nodiscard]] static bool exonMutantOffset( std::shared_ptr<const CodingSequence> coding_seq_ptr,
                                              const VariantMutationOffset& indel_adjust,
                                              StrandSense& strand,
                                              ExonOffsetMap& exon_offset_map);
