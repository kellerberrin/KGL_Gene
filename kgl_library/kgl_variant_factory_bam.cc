...
std::shared_ptr<kgl::GenomeVariant> kgl::BamFactory::readParseBam(const std::string& genome_name,
  ...
  std::shared_ptr<GenomeVariant> genome_single_variants = kgl::GenomeVariant::emptyGenomeVariant(genome_name, GenomeVariant::HAPLOID_GENOME, genome_db_ptr);

  // To be implemented.
  // To be implemented.
  ExecEnv::log().warn("BAM file factory is not yet implmented, 0 variants will be returned for BAM file :{}", bam_file_name);

  ExecEnv::log().info("Bam file {} has: {} raw variants", bam_file_name, genome_single_variants->variantCount());
