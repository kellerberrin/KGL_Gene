...
enum class InfoEvidenceType { Float, Integer, String, FloatArray, IntegerArray, StringArray, Boolean};
...
private:
  ...
  InfoEvidenceIndex(VCFInfoRecord vcfInfoRecord,
                    ...
                    size_t index,
                    std::shared_ptr<InfoEvidenceHeader> info_evidence_header)
  ...
  const size_t index_;
  std::shared_ptr<InfoEvidenceHeader> info_evidence_header_; // Ensure the index knows which header it belongs to.
...
public:
  ...
  [[nodiscard]] bool setupEvidenceHeader(const VCFInfoRecordMap& vcf_info_map);
