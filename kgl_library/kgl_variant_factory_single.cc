    ...
    if (ExtendCountColumns::offsetToNucleotide(idx) != ExtendCountColumns::baseToExtend(reference_nucleotide)
      ...
      if (quality >= variant_quality) {
        ...
        if (ExtendCountColumns::isBaseCode(mutant_nucleotide)) {
          ...
          variant_count += VariantFactory::addGenomeVariant(genome_single_variants, snp_variant_ptr); // Annotate with genome information
          ...
          variant_count += VariantFactory::addGenomeVariant(genome_single_variants, delete_variant_ptr); // Annotate with genome information
  ...
  for(size_t idx = 0; idx <  DNA5::NUCLEOTIDE_COLUMNS; ++idx) {
    ...
    if (DNA5::offsetToNucleotide(idx) != reference_nucleotide
      ...
      if (quality >= variant_quality) {
        ...
        variant_count += VariantFactory::addGenomeVariant(genome_single_variants, insert_variant_ptr); // Annotate with genome information
