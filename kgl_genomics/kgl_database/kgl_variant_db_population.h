...
using ProcessGenomeFunc = std::function<bool(std::shared_ptr<const GenomeDB>, const std::shared_ptr<const Variant>&)>;
...
public:
  ...
  // Note that this version passes the work object by std::shared_ptr<Obj> as each thread updates a common object_ptr.
  template<class Obj> bool processAll_MT(Obj& object, MemberGenomeFunc<Obj> objFunc) const {

    ProcessGenomeFunc callable = std::bind_front(objFunc, &object);
    return processAll_MT(callable);

  }
  // Implementation
  bool processAll_MT(const ProcessGenomeFunc& objFunc) const;
