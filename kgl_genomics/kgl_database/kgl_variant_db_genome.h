...
using ContigVariantMap = std::map<ContigId_t, std::shared_ptr<ContigOffsetVariant>>;
...
public:
  ...
  [[nodiscard]] std::shared_ptr<GenomeVariantArray> filterVariants(const VariantFilter& filter) const;
  ...
  [[nodiscard]] const ContigVariantMap& getMap() const { return contig_map_; }

  // Creates the contig if it does not exist.
  [[nodiscard]] std::optional<std::shared_ptr<ContigOffsetVariant>> getCreateContig(const ContigId_t& contig_id);
  // Const version, Returns nullopt if the contig does not exist.
  [[nodiscard]] std::optional<std::shared_ptr<const ContigOffsetVariant>> getContig(const ContigId_t& contig_id) const;
  // Returns nullopt if the contig does not exist.
  [[nodiscard]] std::optional<std::shared_ptr<ContigOffsetVariant>> getContig(const ContigId_t& contig_id);
...
private:
  ...
  [[nodiscard]] bool addContig(const std::shared_ptr<ContigOffsetVariant>& contig_ptr);
...
bool GenomeVariantArray::processAll(Obj& object, Func objFunc)  const {
