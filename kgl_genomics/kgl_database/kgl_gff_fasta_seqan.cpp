...
class kgl::ParseGffFastaSeqan::GffFastaSeqan {

public:

  GffFastaSeqan() = default;
  ~GffFastaSeqan() = default;

  using FeatureSinkPtr = bool (kgl::ContigReference::*)(std::shared_ptr<kgl::Feature>&);

  void readGffFile(const std::string &gff_file_name, kgl::GenomeReference& genome_db, FeatureSinkPtr feature_sink);

  bool writeFastaFile(const std::string& fasta_file_name, const std::vector<WriteFastaSequence>& fasta_sequences);

  void readFastaFile(const std::string& fasta_file_name, std::shared_ptr<kgl::GenomeReference>& genome_db_ptr);
...
private:
  ...
  bool parseGffRecord(kgl::GenomeReference& genome_db_ptr,
...
bool kgl::ParseGffFastaSeqan::GffFastaSeqan::readFastaFile(const std::string& fasta_file_name,
...
void kgl::ParseGffFastaSeqan::GffFastaSeqan::readFastaFile(const std::string& fasta_file_name,
                                                           std::shared_ptr<kgl::GenomeReference>& genome_db_ptr) {
...
void kgl::ParseGffFastaSeqan::GffFastaSeqan::readGffFile(const std::string &gff_file_name,
...
bool kgl::ParseGffFastaSeqan::GffFastaImpl::parseGffRecord(kgl::GenomeReference& genome_db,
                                                           seqan::GffRecord& record,
                                                           long gff_line_counter,
                                                           FeatureSinkPtr feature_sink) {
  // Get the attributes.
  ...
  std::optional<std::shared_ptr<const kgl::ContigReference>> contig_opt = genome_db.getContigSequence(contig_id);
  if (not contig_opt) {
...
bool kgl::ParseGffFastaSeqan::GffFastaImpl::writeFastaFile(const std::string& fasta_file_name,
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// ParseGffFastaSeqan is a public facade class that passes the functionality onto ParseGffFastaSeqan::GffFastaImpl.
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////


kgl::ParseGffFastaSeqan::ParseGffFastaSeqan() : gff_fasta_impl_ptr_(std::make_unique<GffFastaImpl>()) {}
kgl::ParseGffFastaSeqan::~ParseGffFastaSeqan() {}  // DO NOT DELETE or USE DEFAULT. Required because of incomplete pimpl type.

// Functionality passed to the implmentation.

std::shared_ptr<kgl::GenomeReference> kgl::ParseGffFastaSeqan::readFastaFile(const std::string& organism,
...
bool kgl::ParseGffFastaSeqan::writeFastaFile(const std::string& fasta_file_name,
                                             const std::vector<WriteFastaSequence>& fasta_sequences) {
...
std::shared_ptr<kgl::GenomeReference> kgl::ParseGffFastaSeqan::readFastaGffFile(const std::string& organism,
                                                                                const std::string& fasta_file_name,
                                                                                const std::string& gff_file_name ) {

  std::shared_ptr<GenomeReference> genome_db_ptr = readFastaFile(organism, fasta_file_name);
  gff_fasta_impl_ptr_->readGffFile(gff_file_name, *genome_db_ptr, &ContigReference::addGeneExonFeature);
  return genome_db_ptr;

}


bool kgl::ParseGffFastaSeqan::readFastaFile(const std::string& fasta_file_name, std::vector<ReadFastaSequence>& fasta_sequences) {

  bool result = gff_fasta_impl_ptr_->readFastaFile(fasta_file_name, fasta_sequences);
  for (const auto& sequence: fasta_sequences) {
