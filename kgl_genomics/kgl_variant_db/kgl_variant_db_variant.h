...
/// VariantDBVariant; Indexed by Variant and Genome. Genomes containing the particular variant (Hom/Het counted) are listed.
/// In addition, each Genome that does NOT contain the population variant is listed (homozygous reference allele).
...
public:
  ...
  /// Allele statistics by Variant. If queried by Variant, the sum of the summary terms should equal the number of Genomes.
  [[nodiscard]] AlleleSummmary summaryByVariant(const std::shared_ptr<const Variant>& variant) const;
  /// Allele statistics by Genome. If queried by Genome, the sum of the summary terms should equal the number of Variants.
  [[nodiscard]] AlleleSummmary summaryByGenome(const GenomeId_t& genome) const;
  /// Allele population statistics. The sum of the summary terms should equal the number of Variants x the number of Genomes.
