...
std::optional<std::shared_ptr<kgl::ContigDB>> kgl::GenomeDB::getCreateContig(const ContigId_t& contig_id) {
  ...
  auto [insert_iter, insert_result] = contig_map_.try_emplace(contig_id);
  if (insert_result) {

    // Add the new contig.
    insert_iter->second = std::make_shared<ContigDB>(contig_id);
...
size_t kgl::GenomeDB::variantCount() const {
  ...
  return std::ranges::fold_left(getMap() | std::views::values,
                               size_t{0},
                               [](size_t sum, const auto& contig_ptr) { return sum + contig_ptr->variantCount(); });
...
std::unique_ptr<kgl::GenomeDB> kgl::GenomeDB::viewFilter(const BaseFilter& filter) const {
  ...
  for (const auto& [contig_id, contig_ptr] : getMap()) {
    ...
    if (not filtered_genome_ptr->addContig(filtered_contig)) {
      ...
      ExecEnv::log().error("GenomeDB::viewFilter; Genome: {}, Unable to insert filtered Contig: {}", genomeId(), contig_id);
...
size_t kgl::GenomeDB::trimEmpty() {
  ...
  return std::erase_if(contig_map_, [](const auto& contig_pair) { return contig_pair.second->variantCount() == 0; });
