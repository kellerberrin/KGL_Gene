...
/// Note all contig offsets begin at 0 (not 1 as is the practice in VCF, GFF3 etc).
...
public:
  ...
  explicit ContigRegionFilter(ContigOffset_t start, ContigOffset_t end) : start_(start), end_(end) {
    ...
    filterName(std::format("Contig filter with variants in the half-interval [{}, {})", start_, end_));
...
private:
  ...
  const ContigOffset_t start_;
  const ContigOffset_t end_;
...
public:
  ...
  explicit ContigModifyFilter(ContigOffset_t start, ContigOffset_t end) : start_(start), end_(end) {
    ...
    filterName(std::format("Contig filter with variants that MODIFY DNA in the half-interval [{}, {})", start_, end_));
...
/// This filter is stateless: the upstream delete map is created fresh (function local) within each
/// applyFilter() invocation, so repeated or concurrent invocations cannot share stale state.
...
#endif //KGL_VARIANT_FILTER_DB_CONTIG_H
