...
public:
  ...
  void initialize() noexcept { unfiltered_ = 0; accepted_ = 0; rejected_ = 0; missing_ = 0; }
  void printStats(double filter_level) const;

  void countUnfiltered() noexcept { ++unfiltered_; }
  void countAccepted() noexcept { ++accepted_; }
  void countRejected() noexcept { ++rejected_; }
  void countMissing() noexcept { ++missing_; }

private:

  std::atomic<std::uint64_t> unfiltered_{0};
  ...
  [[nodiscard]] static double percent(std::uint64_t numerator, std::uint64_t denominator) { return denominator > 0 ? 100.0 * (static_cast<double>(numerator) / static_cast<double>(denominator)) : 0.0; }
  [[nodiscard]] double acceptedPercent() const { return percent(accepted_, unfiltered_); }
  [[nodiscard]] double rejectedPercent() const { return percent(rejected_, unfiltered_); }
  [[nodiscard]] double missingPercent() const { return percent(missing_, unfiltered_); }
  [[nodiscard]] double rejectionRate() const { return percent(rejected_, accepted_ + rejected_); }
...
/// Filters for variants with AF/MLEAF values >= frequency_cutoff.
/// Note: a missing AF/MLEAF field accepts the variant (counted as missing); a malformed vector rejects it.
class P7FrequencyFilter : public FilterVariants {

public:

  P7FrequencyFilter(FreqInfoField info_field, double freq_cutoff)
      : info_field_(info_field == FreqInfoField::AF ? AF_FIELD_ : MLEAF_FIELD_), freq_cutoff_(freq_cutoff) { filterName("P7FrequencyFilter"); }
...
private:
  ...
  const std::string info_field_;
  const double freq_cutoff_;
...
private:
  ...
  // Outcome of a single info-field threshold check.
  enum class FieldOutcome { MISSING, PASS, FAIL };
  ...
  [[nodiscard]] static FieldOutcome checkInfoField(const Variant& variant, const char* field, double level, bool pass_if_greater_equal);

  // Global filter statistics.
  inline static std::atomic<std::uint64_t> unfiltered_variants_{0};
  inline static std::atomic<std::uint64_t> accepted_variants_{0};
