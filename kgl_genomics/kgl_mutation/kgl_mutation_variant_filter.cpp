...
// Also returns the number of multiple variants found at each offset which are filtered to a single variant.
std::pair<kgl::OffsetVariantMap, kgl::FilteredVariantStats>
kgl::SequenceVariantFilter::getFrameshiftAdjusted(const std::shared_ptr<const ContigDB>& contig_ptr,
                                                  const OpenRightUnsigned& variant_interval) {
  ...
  // Remove all frameshift indels.
  modify_contig_ptr = modify_contig_ptr->viewFilter(NotFilter(FrameShiftFilter()));
...
// Also returns the number of multiple variants found at each offset which are filtered to a single variant.
std::pair<kgl::OffsetVariantMap, kgl::FilteredVariantStats>
kgl::SequenceVariantFilter::getSNPAdjusted(const std::shared_ptr<const ContigDB>& contig_ptr,
                                                  const OpenRightUnsigned& variant_interval) {
  ...
  // Remove all frameshift indels.
  modify_contig_ptr = modify_contig_ptr->viewFilter(SNPFilter());
...
void kgl::SequenceVariantFilter::FrameshiftAdjusted(const std::shared_ptr<const ContigDB>& contig_ptr,
                                                    const OpenRightUnsigned& sequence_interval) {


  const auto [interval_map, filter_stats] = SequenceVariantFilter::getFrameshiftAdjusted(contig_ptr,
                                                                                         sequence_interval);
  offset_variant_map_ = std::move(interval_map);
  variant_filter_stats_ = filter_stats;
...
void kgl::SequenceVariantFilter::SNPAdjusted(const std::shared_ptr<const ContigDB>& contig_ptr,
                                             const OpenRightUnsigned& sequence_interval) {
...
void kgl::SequenceVariantFilter::selectFilterType(const std::shared_ptr<const ContigDB>& contig_ptr,
  ...
  switch(sequenceFilterType()) {
    ...
    case SeqVariantFilterType::FRAMESHIFT_ADJUSTED:
      FrameshiftAdjusted(contig_ptr, sequence_interval);
      break;

    case SeqVariantFilterType::SNP_ADJUSTED:
      SNPAdjusted(contig_ptr, sequence_interval);
      break;
