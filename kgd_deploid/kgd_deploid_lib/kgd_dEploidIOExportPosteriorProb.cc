...
void McmcMachinery::writeLastFwdProb(bool useIBD){
    ...
    for ( size_t tmpk = 0; tmpk < kStrain_; tmpk++ ){
        ...
        for ( size_t chromi = 0 ; chromi < dEploidIO_->indexOfChromStarts_.size(); chromi++ ){
            ...
            updatingSingle.core ( dEploidIO_->refCount_, dEploidIO_->altCount_, dEploidIO_->plaf_, currentExpectedWsaf_, currentProp_, currentHap_);
        ...
        for ( size_t ii = 0; ii < probabilities[0].size(); ii++){
            if (doAllowInbreeding() == true){
                if ( ii <= (panelSize - kStrain()) ){
                    ...
                    ofstreamExportFwdProb << "I" << (ii)-(panelSize - kStrain()) ;
                }
            } else {
