...
void DEploidIO::writeRecombProb (std::shared_ptr<Panel> panel ){
...
void DEploidIO::writeLog ( ostream * writeTo ){
    size_t nHash = 30 + string(kgd::DeploidExecEnv::VERSION).size();
    for ( size_t i = 0; i < nHash; i++){
        (*writeTo) << "#";
    }
    (*writeTo) << "\n";
    (*writeTo) << "#        kgd_deploid "<< setw(10) << kgd::DeploidExecEnv::VERSION << " log        #\n";
    for ( size_t i = 0; i < nHash; i++){
        (*writeTo) << "#";
    }
    (*writeTo) << "\n";
    (*writeTo) << "Program was compiled on: " << compileTime_ << endl;
    (*writeTo) << "kgd_deploid version: " << dEploidGitVersion_ << endl;
    (*writeTo) << "\n";
    (*writeTo) << "Input data: \n";
