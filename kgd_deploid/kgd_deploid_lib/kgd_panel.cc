...
void Panel::checkForExceptions( size_t nLoci, std::string panelFileName ){
...
void Panel::buildExamplePanel1(){
    this->chrom_ = std::vector <std::string> ({"Pf3D7_01_v3"});
    this->position_.push_back( std::vector <int> ({93157, 94422, 94459, 94487, 95518, 95632, 95641 }) );
    this->indexOfChromStarts_ = std::vector <size_t> ({0});
    this->buildExamplePanelContent();
}


void Panel::buildExamplePanel2(){
    this->chrom_ = std::vector <std::string> ({"Pf3D7_01_v3", "Pf3D7_02_v3", "Pf3D7_03_v3"});
    this->position_.push_back( std::vector <int> ({93157}) );
    this->position_.push_back( std::vector <int> ({94422, 94459, 94487, 95518, 95632}) );
    this->position_.push_back( std::vector <int> ({95641 }) );
    this->indexOfChromStarts_ = std::vector <size_t> ({0, 1, 6});
    this->buildExamplePanelContent();
}


void Panel::buildExamplePanelContent(){
    this->content_.push_back( std::vector <double> ({0,0,0,1}) );
    this->content_.push_back( std::vector <double> ({0,0,0,1}) );
    this->content_.push_back( std::vector <double> ({0,0,0,1}) );
    this->content_.push_back( std::vector <double> ({0,0,0,1}) );
    this->content_.push_back( std::vector <double> ({0,1,1,0}) );
    this->content_.push_back( std::vector <double> ({0,0,1,0}) );
    this->content_.push_back( std::vector <double> ({0,0,1,0}) );
...
void Panel::updatePanelWithHaps(size_t inbreedingPanelSizeSetTo, size_t excludedStrain, std::vector < std::vector<double> > & haps){
    this->setInbreedingPanelSize(inbreedingPanelSizeSetTo);
