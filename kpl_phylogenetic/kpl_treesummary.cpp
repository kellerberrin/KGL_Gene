...
std::string kpl::TreeSummary::getNewick(unsigned index) {
...
void kpl::TreeSummary::readTreefile(const std::string filename, unsigned skip) {
  TreeManip tm;
...
void kpl::TreeSummary::showSummary() const {
  ...
  for (auto &key_value_pair : _treeIDs) {
    ...
    std::copy(key_value_pair.second.begin(), key_value_pair.second.end(),
