...
void kpl::Data::setPartition(Partition::SharedPtr partition) {
...
kpl::Partition::SharedPtr kpl::Data::getPartition() {
...
unsigned kpl::Data::getNumSubsets() const {
...
std::string kpl::Data::getSubsetName(unsigned subset) const {
...
const kpl::Data::partition_key_t& kpl::Data::getPartitionKey() const {
...
const kpl::Data::pattern_counts_t& kpl::Data::getPatternCounts() const {
...
const kpl::Data::monomorphic_vect_t& kpl::Data::getMonomorphic() const {
...
const kpl::Data::taxon_names_t& kpl::Data::getTaxonNames() const {
...
const kpl::Data::data_matrix_t& kpl::Data::getDataMatrix() const {
...
kpl::Data::begin_end_pair_t kpl::Data::getSubsetBeginEnd(unsigned subset) const {
...
unsigned kpl::Data::getNumPatterns() const {
...
kpl::Data::npatterns_vect_t kpl::Data::calcNumPatternsVect() const {
...
unsigned kpl::Data::getNumStatesForSubset(unsigned subset) const {
...
unsigned kpl::Data::getNumPatternsInSubset(unsigned subset) const {
...
unsigned kpl::Data::getNumTaxa() const {
...
unsigned kpl::Data::calcSeqLen() const {
...
unsigned kpl::Data::calcSeqLenInSubset(unsigned subset) const {
...
unsigned kpl::Data::buildSubsetSpecificMaps(unsigned ntaxa, unsigned seqlen, unsigned nsubsets) {
...
void kpl::Data::updatePatternMap(Data::pattern_vect_t &pattern, unsigned subset) {
...
void kpl::Data::compressPatterns() {
...
unsigned kpl::Data::storeTaxonNames(NxsTaxaBlock *taxaBlock, unsigned taxa_block_index) {
...
unsigned kpl::Data::storeData(unsigned ntax, unsigned nchar_before, NxsCharactersBlock *charBlock,
                                NxsCharactersBlock::DataTypesEnum datatype) {
  ...
  // remember that sites begin at 1, not 0, in partition definitions
  ...
  if (datatype == NxsCharactersBlock::dna || datatype == NxsCharactersBlock::rna ||
    ...
    if (dt.isCodon()) {
      ...
      if (!dt.isNucleotide()) {
        throw XStrom(boost::format(
            "Partition subset has data type \"%s\" but data read from file has data type \"nucleotide\"") %
    ...
    if (!dt.isProtein()) {
      ...
      throw XStrom(
          boost::format("Partition subset has data type \"%s\" but data read from file has data type \"protein\"") %
          dt.getDataTypeAsString());
    ...
    if (!dt.isStandard()) {
      throw XStrom(
          boost::format("Partition subset has data type \"%s\" but data read from file has data type \"standard\"") %
          dt.getDataTypeAsString());
    }
...
void kpl::Data::getDataFromFile(const std::string filename) {
...
std::string kpl::Data::createTaxaBlock() const {
...
std::string kpl::Data::createTranslateStatement() const {
