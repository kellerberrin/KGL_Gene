...
bool kpl::Strom::parseCommandLine(int argc, const char **argv) {
  ...
  desc.add_options()
      ...
      ("topopriorC", boost::program_options::value(&_topo_prior_C)->default_value(1.0), "topology prior C: tree (or resolution class) with m internal nodes has probfailure C time greater than tree (or resolution class) with m+1 internal nodes.")
...
bool kpl::Strom::splitAssignmentString(const std::string & definition, std::vector<std::string> & vector_of_subset_names, std::vector<double>  & vector_of_values) {
...
void kpl::Strom::sample(unsigned iteration, Chain & chain) {
...
void kpl::Strom::showPartitionInfo() {
...
void kpl::Strom::calcHeatingPowers() {
...
void kpl::Strom::startTuningChains() {
  _swaps.assign(_num_chains*_num_chains, 0);
...
void kpl::Strom::stopTuningChains() {
...
void kpl::Strom::stepChains(unsigned iteration, bool sampling) {
...
void kpl::Strom::showChainTuningInfo() const {
...
void kpl::Strom::swapSummary() const {
