...
void kgl::MutateAnalysis::printMutationAnalysis(const std::string& file_name) const {

  std::ofstream out_file(file_name);

  if (not out_file.good()) {

    ExecEnv::log().error("utateAnalysis::printMutationAnalysis; unable to open output file: {}", file_name);
    return;
...
kgl::MultipleVariantMap kgl::MutateGenes::multipleOffsetVariants(const std::shared_ptr<const PopulationDB>& population_ptr) {
...
std::multimap<double, std::tuple<kgl::GenomeId_t, size_t, size_t>>
    kgl::MutateGenes::genomeSummary( const MultipleVariantMap& multiple_variant_map,
                                     const std::shared_ptr<const PopulationDB>& population_ptr) {
...
void kgl::MutateGenes::printMultipleAllele(const std::shared_ptr<const PopulationDB>& population_ptr,
  ...
  if (not write_file.good()) {
    ...
    ExecEnv::log().error("MutateGenes::printMultipleAllele; unable to open output file: {}", file_name);
    return;

  }

  auto multiple_variant_map = multipleOffsetVariants(population_ptr);
  auto genome_summary_map = genomeSummary(multiple_variant_map, population_ptr);

  // Summarize which genomes have the most multiple variants.
  write_file << "genome" << delimiter << "Proportion Multiple" << delimiter<< "Multiple Count" << delimiter << "Variant Count" << '\n';
  std::ranges::reverse_view rev_map {genome_summary_map};
  for (auto const& [proportion, count_tuple] : rev_map) {

    auto const& [genome_id, multiple_count, variant_count] = count_tuple;
    write_file << genome_id << delimiter << (proportion * 100.0) << delimiter<<  multiple_count << delimiter << variant_count << '\n';
