...
double kol::MazanduSimUICSetSimilarity::calculateSimilarity(const OntologySetType<std::string> &row_terms,
...
double kol::MazanduSimUICSetSimilarity::calcICSum(const OntologySetType<std::string> &terms) const {

  double sum{0.0};
  for (auto const &term : terms) {

    sum += ic_map_ptr_->getValue(term);
