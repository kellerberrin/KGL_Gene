...
// Add to the genome variant pointer.
  for (const auto &variant_map : aggregated_variants_vec) {
    ...
    if (compound_variant_ptr) {
      ...
      VariantFactory::addVariantToGenome( compound_variants, compound_variant_ptr);
...
bool kgl::InsertDeleteFactory::aggregateVariants(const std::shared_ptr<const GenomeVariant>& variant_ptr,
  ...
  for (const auto& contig_variants : variant_ptr->getMap()) {  // For all contigs,
    ...
    for (const auto& variant : contig_variants.second->getMap()) {  // For all variants.
      ...
      if (selectVariant(variant.second) and variant.second->isSingle()) {  // The virtual selection function.
