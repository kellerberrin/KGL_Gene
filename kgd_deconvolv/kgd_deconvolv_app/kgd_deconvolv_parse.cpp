...
void getFilePath(const std::string& option_text,
  ...
  } else if (file_name == kgd::DeconvolvArgs::NOT_SPECIFIED) {
  ...
  if (!file_exists) {
    ...
    kellerberrin::ExecEnv::log().critical("Option --{}, File: {} does not exist.", option_text, file_path.string());

  }

  if (error_code.value() != boost::system::errc::success) {

    kellerberrin::ExecEnv::log().critical("Option --{}, File: {}, error: {}.", option_text, file_path.string(), error_code.message());
...
bool kgd::Deconvolv::parseCommandLine(int argc, char const ** argv) {
  ...
  setShortDescription(parser, "kgd_deconvolv COI haplotype generation");
  setVersion(parser, VERSION);
  setDate(parser, "May 2018");

  const char* program_desc =
  R"(kgd_deconvolv Complexity Of Infection (COI) haplotype generation)";
