  ...
  while (j > 0) {
    ...
    for (size_t i = 0; i < nPanel_; i++) {
      ...
      for (size_t ii = 0; ii < nPanel_; ii++) {
        ...
        bwdTmp[i] += emission_[j][panel_->getContentIndex(hapIndexBack,ii)] * bwdProbs_.back()[ii] * pRecEachHap;

        if (i == ii) {

          bwdTmp[i] += emission_[j][panel_->getContentIndex(hapIndexBack,ii)] * bwdProbs_.back()[ii] * pNoRec;
...
void kgd::UpdateSingleHap::calcFwdProbs() {
  ...
  for (size_t i = 0; i < nPanel_; i++) {
    ...
    auto panel_index = panel_->getContentIndex(hapIndex, i);
  ...
  for (size_t j = 1; j < nLoci_; j++) {
    ...
    for (size_t i = 0; i < nPanel_; i++) {
      ...
      fwdTmp[i] = emission_[j][panel_->getContentIndex(hapIndex,i)] * (fwdProbs_.back()[i] * pNoRec + massFromRec);
