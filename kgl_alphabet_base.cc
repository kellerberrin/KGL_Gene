//
// Created by kellerberrin on 31/10/17.
//


#include "kgl_alphabet_base.h"

namespace kgl = kellerberrin::genome;

// Instantiate the NucleotideColumn_DNA5 constants for the edification of the linker.

const kgl::ContigOffset_t kgl::NucleotideColumn_DNA5::NUCLEOTIDE_COLUMNS;
const kgl::Nucleotide_ExtendedDNA5 kgl::NucleotideColumn_DNA5::A_NUCLEOTIDE;
const kgl::Nucleotide_ExtendedDNA5 kgl::NucleotideColumn_DNA5::A_NUCLEOTIDE_LC;
const kgl::ContigOffset_t kgl::NucleotideColumn_DNA5::A_NUCLEOTIDE_OFFSET;
const kgl::Nucleotide_ExtendedDNA5 kgl::NucleotideColumn_DNA5::C_NUCLEOTIDE;
const kgl::Nucleotide_ExtendedDNA5 kgl::NucleotideColumn_DNA5::C_NUCLEOTIDE_LC;
const kgl::ContigOffset_t kgl::NucleotideColumn_DNA5::C_NUCLEOTIDE_OFFSET;
const kgl::Nucleotide_ExtendedDNA5 kgl::NucleotideColumn_DNA5::G_NUCLEOTIDE;
const kgl::Nucleotide_ExtendedDNA5 kgl::NucleotideColumn_DNA5::G_NUCLEOTIDE_LC;
const kgl::ContigOffset_t kgl::NucleotideColumn_DNA5::G_NUCLEOTIDE_OFFSET;
const kgl::Nucleotide_ExtendedDNA5 kgl::NucleotideColumn_DNA5::U_NUCLEOTIDE;
const kgl::Nucleotide_ExtendedDNA5 kgl::NucleotideColumn_DNA5::U_NUCLEOTIDE_LC;
const kgl::ContigOffset_t kgl::NucleotideColumn_DNA5::U_NUCLEOTIDE_OFFSET;
const kgl::Nucleotide_ExtendedDNA5 kgl::NucleotideColumn_DNA5::T_NUCLEOTIDE;
const kgl::Nucleotide_ExtendedDNA5 kgl::NucleotideColumn_DNA5::T_NUCLEOTIDE_LC;
const kgl::ContigOffset_t kgl::NucleotideColumn_DNA5::T_NUCLEOTIDE_OFFSET;
const kgl::Nucleotide_ExtendedDNA5 kgl::NucleotideColumn_DNA5::N_NUCLEOTIDE;
const kgl::Nucleotide_ExtendedDNA5 kgl::NucleotideColumn_DNA5::N_NUCLEOTIDE_LC;
const kgl::ContigOffset_t kgl::NucleotideColumn_DNA5::N_NUCLEOTIDE_OFFSET;
const kgl::Nucleotide_ExtendedDNA5 kgl::NucleotideColumn_DNA5::DELETE_NUCLEOTIDE;
const kgl::ContigOffset_t kgl::NucleotideColumn_DNA5::DELETE_NUCLEOTIDE_OFFSET;
const kgl::Nucleotide_ExtendedDNA5 kgl::NucleotideColumn_DNA5::INSERT_A_NUCLEOTIDE;
const kgl::ContigOffset_t kgl::NucleotideColumn_DNA5::INSERT_A_NUCLEOTIDE_OFFSET;
const kgl::Nucleotide_ExtendedDNA5 kgl::NucleotideColumn_DNA5::INSERT_C_NUCLEOTIDE;
const kgl::ContigOffset_t kgl::NucleotideColumn_DNA5::INSERT_C_NUCLEOTIDE_OFFSET;
const kgl::Nucleotide_ExtendedDNA5 kgl::NucleotideColumn_DNA5::INSERT_G_NUCLEOTIDE;
const kgl::ContigOffset_t kgl::NucleotideColumn_DNA5::INSERT_G_NUCLEOTIDE_OFFSET;
const kgl::Nucleotide_ExtendedDNA5 kgl::NucleotideColumn_DNA5::INSERT_U_NUCLEOTIDE;
const kgl::ContigOffset_t kgl::NucleotideColumn_DNA5::INSERT_U_NUCLEOTIDE_OFFSET;
const kgl::Nucleotide_ExtendedDNA5 kgl::NucleotideColumn_DNA5::INSERT_T_NUCLEOTIDE;
const kgl::ContigOffset_t kgl::NucleotideColumn_DNA5::INSERT_T_NUCLEOTIDE_OFFSET;
const kgl::Nucleotide_ExtendedDNA5 kgl::NucleotideColumn_DNA5::INSERT_N_NUCLEOTIDE;
const kgl::ContigOffset_t kgl::NucleotideColumn_DNA5::INSERT_N_NUCLEOTIDE_OFFSET;

