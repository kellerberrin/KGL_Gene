...
bool kgl::GenomePhasing::haploidPhasing(size_t vcf_ploidy,
  ...
  for (auto const& [genome_id, genome_ptr] : unphased_population_ptr->getMap()) {
    ...
    std::optional<std::shared_ptr<GenomeDB>> genome_variant_opt = haploid_population->getCreateGenome(genome_id);
...
std::shared_ptr<kgl::PopulationDB> kgl::GenomePhasing::filterClonal(const std::string& phase_file,
  ...
  std::shared_ptr<PopulationDB> filtered_population(std::make_shared<PopulationDB>(filtered_name, unphased_population_ptr->dataSource()));
...
bool kgl::GenomePhasing::analyseCountStatistics(const OffsetDB& unphased_vector, size_t& phase_index) {
