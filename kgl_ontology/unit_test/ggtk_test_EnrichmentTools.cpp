...
BOOST_FIXTURE_TEST_SUITE(TestEnrichmentSuite, TestEnrichment)
...
BOOST_AUTO_TEST_CASE(test_enrichment_tools_hypergeometric_test)
{
  ...
  auto p_val = EnrichmentTools::oneSidedRawPvalue_hyper(8,10,17,7);
...
BOOST_AUTO_TEST_CASE(test_enrichment_tools_child_genes)
{
  ...
  auto term_genes = EnrichmentTools::getDescendantGenes(goGraph(), annotation(), "GO:0002507");
...
BOOST_AUTO_TEST_CASE(test_enrichment_tools_enrichment_significance)
{
  ...
  auto term_genes = EnrichmentTools::getDescendantGenes(goGraph(), annotation(), "GO:0002507");
  auto p_val = EnrichmentTools::enrichmentSignificance(goGraph(), annotation(), term_genes, "GO:0002507");
