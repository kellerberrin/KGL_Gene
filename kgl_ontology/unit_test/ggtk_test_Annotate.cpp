...
BOOST_AUTO_TEST_CASE(test_go_terms_for_gene)
{
  ...
  if (convertSet(SetUtilities::convert_vector(annotation().getGoTermsForGene("A0A024R161"))) != go_annotations) BOOST_FAIL("Found invalid go terms for gene" );
...
BOOST_AUTO_TEST_CASE(test_genes_for_go_term)
{
  ...
  if (convertSet(SetUtilities::convert_vector(annotation().getGenesForGoTerm("GO:0000015"))) != gene_set) BOOST_FAIL("Invalid gene list for go term" );
